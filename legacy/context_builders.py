"""
contextor/core/single_file/context_builders.py
...
from contextor.core.analysis.export_analysis import (
    extract_exports,
...
from contextor.core.symbol_engine import (
    build_symbol_index,
    classify_imports,
...
def collect_symbol_context(file_path, modules, module_id, root_path):
    ...
    references = build_symbol_references(modules, all_symbols, root_path, definer_module=module_id)
...
def collect_import_context(module, modules):
    known_modules = set(modules.keys()) | {
        key.replace("core.", "contextor.core.") for key in modules.keys() if key.startswith("core.")
    }
...
def collect_architecture_context(module_id, project_graph, global_report=None, modules=None):
    ...
    if modules:
        ...
        for error in all_collisions:
            if any(
                module_id in node or node.endswith(module_id.replace(".", "/"))
    ...
    return {
        ...
        "signals": architecture_signals(
            module_id, hard_edges, soft_edges, hotspots, cycles, metrics["nodes"]
        ),
