...
from repo_guardian.ui.theme import HeaderTooltipManager, BG, SURFACE, BORDER, TEXT, PRIMARY, PAD_SM, PAD_MD, PAD_LG
from repo_guardian.ui.path_memory import load_state, save_state
from repo_generator.config import DEFAULT_EXTENSIONS, DEFAULT_SKIP_DIRS
...
class FilterWindow:
    def __init__(self, parent_gui):
        ...
        self.preset_label_var = tk.StringVar(master=self.window, value=f"Active Preset: {self.parent_gui.current_preset}" if self.parent_gui.current_preset else "Active Preset: None")
        ttk.Label(self.window, textvariable=self.preset_label_var, style="Sub.TLabel", foreground=PRIMARY).pack(
            anchor="w", padx=PAD_LG, pady=(PAD_MD, 0)
        )

        filter_sub = ttk.Label(self.window, text="Configure extensions and directories to IGNORE", style="Sub.TLabel")
        filter_sub.pack(anchor="w", padx=PAD_LG, pady=(0, PAD_SM))
        self.f_tooltip = HeaderTooltipManager(filter_sub, "Configure extensions and directories to IGNORE")

        ttk.Label(self.window, text="File extensions (✓ = IGNORE)", font=("Segoe UI", 11, "bold")).pack(anchor="w", padx=PAD_LG, pady=(PAD_MD, PAD_SM))
        ...
        for index, ext in enumerate(sorted(DEFAULT_EXTENSIONS)):
            ...
            ttk.Checkbutton(ext_frame, text=ext, variable=var).grid(row=index // 5, column=index % 5, sticky="w", padx=10, pady=2)

        ttk.Label(self.window, text="Skipped directories (✓ = IGNORE)", font=("Segoe UI", 11, "bold")).pack(anchor="w", padx=PAD_LG, pady=(PAD_MD, PAD_SM))
        ...
        for index, directory in enumerate(sorted(DEFAULT_SKIP_DIRS)):
            ...
            ttk.Checkbutton(dir_frame, text=directory, variable=var).grid(row=index // 5, column=index % 5, sticky="w", padx=10, pady=2)
        ...
        b_def_pre = ttk.Button(preset_buttons, text="Default", command=self.load_default_preset, style="Secondary.TButton")
        b_def_pre.pack(side=tk.LEFT, padx=(0, PAD_SM))
        self.f_tooltip.bind_tooltip(b_def_pre, "Restore to default (all checkboxes checked, no active preset).")

        b_save_pre = ttk.Button(preset_buttons, text="Save Preset", command=self.save_preset, style="Secondary.TButton")
        b_save_pre.pack(side=tk.LEFT, padx=(0, PAD_SM))
        self.f_tooltip.bind_tooltip(b_save_pre, "Save the current filter configuration as a reusable preset.")
        
        b_load_pre = ttk.Button(preset_buttons, text="Load Preset", command=self.load_preset, style="Secondary.TButton")
        ...
        b_del_pre = ttk.Button(preset_buttons, text="Delete Preset", command=self.delete_preset, style="Danger.Ghost.TButton")
        ...
        btn_sa_f = ttk.Button(buttons, text="Select all", command=self.select_all_filters, style="Ghost.TButton")
        btn_sa_f.pack(side=tk.LEFT, padx=(0, PAD_SM))
        self.f_tooltip.bind_tooltip(btn_sa_f, "Check all extensions and directories.")

        btn_ua_f = ttk.Button(buttons, text="Unselect all", command=self.clear_filters, style="Ghost.TButton")
        btn_ua_f.pack(side=tk.LEFT)
        self.f_tooltip.bind_tooltip(btn_ua_f, "Uncheck all extensions and directories.")

        btn_sv_f = ttk.Button(buttons, text="Confirm", command=lambda: self.handle_close(is_confirm=True), style="Primary.TButton")
    ...
    def save_preset(self):
        ...
        self.save_dlg.configure(bg=BG)
        self.save_dlg.transient(self.window)
        self.save_dlg.grab_set()
        ttk.Label(self.save_dlg, text="Existing presets:", style="Sub.TLabel").pack(anchor="w", padx=PAD_MD, pady=(PAD_MD, PAD_SM))
        listb = tk.Listbox(self.save_dlg, bg=SURFACE, fg=TEXT, selectbackground=PRIMARY, selectforeground="#ffffff", borderwidth=0, highlightthickness=1, highlightbackground=BORDER, font=("Segoe UI", 10), height=8)
        listb.pack(fill=tk.BOTH, expand=True, padx=PAD_MD)
        for p in sorted(presets.keys()):
            listb.insert(tk.END, p)
        ttk.Label(self.save_dlg, text="Preset name:", style="Sub.TLabel").pack(anchor="w", padx=PAD_MD, pady=(PAD_SM, 2))
        ...
        def do_save():
            ...
            if name.lower() == "default":
                messagebox.showwarning("Error", "The name 'default' is reserved and read-only.", parent=self.save_dlg)
                return
            if name in presets:
                if not messagebox.askyesno("Overwrite?", f"Preset '{name}' already exists.\nDo you want to overwrite it?", parent=self.save_dlg):
                    return
        ...
        ttk.Button(btn_row, text="Cancel", command=self.save_dlg.destroy, style="Ghost.TButton").pack(side=tk.RIGHT, padx=(0, PAD_SM))
    ...
    def load_preset(self):
        ...
        self.load_dlg.configure(bg=BG)
        self.load_dlg.transient(self.window)
        self.load_dlg.grab_set()
        listb = tk.Listbox(self.load_dlg, bg=SURFACE, fg=TEXT, selectbackground=PRIMARY, selectforeground="#ffffff", borderwidth=0, highlightthickness=1)
        ...
        ttk.Button(self.load_dlg, text="Load", command=load_btn, style="Primary.TButton").pack(pady=PAD_SM)
    ...
    def delete_preset(self):
        ...
        if not presets:
            ...
            return
        def on_select(name):
            confirm = messagebox.askyesno("Confirm Delete", f"Delete preset '{name}'?", parent=self.del_dlg)
            if confirm:
                del presets[name]
                save_state(repo_gui_presets=presets)
        ...
        self.del_dlg.configure(bg=BG)
        self.del_dlg.transient(self.window)
        self.del_dlg.grab_set()
        listb = tk.Listbox(self.del_dlg, bg=SURFACE, fg=TEXT, selectbackground=PRIMARY, selectforeground="#ffffff", borderwidth=0, highlightthickness=1)
        listb.pack(fill=tk.BOTH, expand=True, padx=PAD_SM, pady=PAD_SM)
        for p in sorted(presets.keys()):
            listb.insert(tk.END, p)
        def del_btn():
            sel = listb.curselection()
            if sel: on_select(listb.get(sel[0]))
        ttk.Button(self.del_dlg, text="Delete", command=del_btn, style="Danger.Ghost.TButton").pack(pady=PAD_SM)
    ...
    def select_all_filters(self):
        for var in self.ext_vars.values(): var.set(True)
        for var in self.dir_vars.values(): var.set(True)

    def clear_filters(self):
        for var in self.ext_vars.values(): var.set(False)
        for var in self.dir_vars.values(): var.set(False)
    ...
    def handle_close(self, is_confirm=False):
        ...
        changed_from_start = (current_exts != self.original_skip_exts or current_skips != self.original_skip_dirs)
        ...
        if self.parent_gui.current_preset:
            ...
            if self.parent_gui.current_preset in presets:
                ...
                if preset_changed:
                    ans = messagebox.askyesnocancel("Update Preset?", f"Preset '{self.parent_gui.current_preset}' has unsaved changes.\n\nYES = Update preset and apply changes.\nNO = Discard changes and close.\nCANCEL = Return to window.", parent=self.window)
                    if ans is None: return
                    if ans is True:
                        presets[self.parent_gui.current_preset] = {"skip_ext": list(current_exts), "skip": list(current_skips)}
                        save_state(repo_gui_presets=presets)
                    ...
                    save_state(repo_gui_skip_exts=list(self.parent_gui.skip_exts), repo_gui_skip_dirs=list(self.parent_gui.skip_dirs), repo_gui_current_preset=self.parent_gui.current_preset)
                    self.window.destroy()
                    return
