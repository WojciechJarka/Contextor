...
from repo_generator.icons import create_icon_button
from repo_generator.filters import FilterWindow
...
class RepoGenerator:
    ...
    def build_gui(self):
        ttk.Label(
            self.root, text="Repo Builder", style="Header.TLabel"
        ).pack(anchor="w", padx=PAD_LG, pady=(PAD_LG, 0))
        ...
        self.listbox = tk.Listbox(
            frame, selectmode=tk.MULTIPLE, exportselection=False,
            yscrollcommand=scrollbar.set, bg=SURFACE, fg=TEXT,
        ...
        self.info_label = ttk.Label(info_frame, text="Estimated file size: 0 KB | Estimated lines: 0", style="Sub.TLabel")
        self.info_label.pack(side=tk.LEFT)

        controls = ttk.Frame(frame)
        controls.pack(side=tk.BOTTOM, fill=tk.X)

        btn_sa = ttk.Button(controls, text="Select all", command=self.select_all, style="Ghost.TButton")
        btn_sa.pack(side=tk.LEFT, padx=(0, PAD_SM))
        self.tooltip.bind_tooltip(btn_sa, "Select all files in the list for generation.")

        btn_ua = ttk.Button(controls, text="Unselect all", command=self.unselect_all, style="Ghost.TButton")
        ...
        btn_ff = ttk.Button(bottom, text="File filters", command=self.open_filters, style="Secondary.TButton")
        ...
        btn_of = ttk.Button(bottom, text="Output Folder", command=self.open_output_folder, style="Secondary.TButton")
        ...
        btn_eo = ttk.Button(bottom, text="Empty output", command=self.clear_output, style="Danger.Ghost.TButton")
        ...
        btn_gen = ttk.Button(self.root, text="Generate repository", command=self.generate, style="Primary.TButton")
    ...
    def on_paste(self, event):
        ...
        if added > 0:
            self.select_all()
            self.update_estimation_info()
            messagebox.showinfo("Paste Paths", f"Successfully pasted and added {added} valid files.", parent=self.root)
        else:
            messagebox.showinfo("Paste Paths", "No valid file paths found in clipboard.", parent=self.root)
    ...
    def on_delete_key(self, event):
        ...
        ans = messagebox.askyesnocancel("Delete files", f"Are you sure you want to remove {count} files from the list?", parent=self.root)
        if ans is True:  # Yes
    ...
    def update_estimation_info(self):
        ...
        self.info_label.config(text=f"Estimated file size: {size_kb:.2f} KB | Estimated lines: {total_lines}")
    ...
    def add_repository(self):
        ...
        for root, dirs, files in os.walk(folder):
            dirs[:] = [d for d in dirs if not any(d.lower() == skip.lower() for skip in self.skip_dirs)]
        ...
        messagebox.showinfo("Repo added", f"Files added: {added}\nSkipped: {blocked}", parent=self.root)
    ...
    def clear_output(self):
        if not os.path.exists(self.output_dir):
            return
        confirm = messagebox.askyesno("Empty output", "Are you sure you want to clear output?\nThis operation cannot be undone.", parent=self.root)
        if not confirm:
            return
