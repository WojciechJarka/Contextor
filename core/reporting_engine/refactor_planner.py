...
"""
repo_guardian/core/reporting_engine/refactor_planner.py
...
def _compute_refactor_plan(
    ...
    for h in hotspots[:10]:
        ...
        if hotspot_type in ("HOTSPOT", "OUTBOUND_HOTSPOT", "HUB"):
            plan.append({
                ...
                "priority": "HIGH" if combined_score >= thresholds.get("critical_score", 0.85) else "MEDIUM",
                "reason": {
    ...
    for cluster in clusters:
        if len(cluster) >= thresholds["refactor_cluster_size"] and len(cluster) < total_modules * 0.6:
            plan.append({
                "type": "SPLIT_PACKAGE",
