...
"""
repo_guardian/core/context/dependencies.py
...
def find_dependents(module_id: str, hard_edges: dict) -> list[str]:
    result = {
        source for source, targets in hard_edges.items() if module_id in targets
    }
    return sorted(result)

def find_soft_dependents(module_id: str, soft_edges: dict) -> list[str]:
    result = {
        source for source, targets in soft_edges.items() if module_id in targets
    }
    return sorted(result)
...
def find_transitive_dependents(module_id: str, hard_edges: dict, max_depth: int = 6) -> list:
    ...
    return [
        {"module": m, "depth": d}
        for m, d in sorted(visited.items(), key=lambda x: x[1])
    ]
