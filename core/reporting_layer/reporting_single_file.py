...
def _build_llm_summary(ctx: dict) -> dict:
    ...
    if "cycle_member" in signals:
        guidance.append("CRITICAL: File creates an import cycle. If modifying it, try using Dependency Inversion or extracting a common interface.")
    if "hotspot" in signals:
        guidance.append("WARNING: File is marked as a HOTSPOT. It has a high change coefficient and many dependencies. Regression risk is elevated.")
    if "config_hub" in signals:
        guidance.append("INFO: Module appears to be a configuration hub. High level of outgoing dependencies is natural here. Focus on data consistency.")
    if blast_radius > 15:
        guidance.append(f"WARNING: Blast radius is {blast_radius} modules. Do not change the public API without thorough testing (or maintain backward compatibility).")
    if not guidance:
        guidance.append("Module is stable (no elevated architectural risk). You can modify its API normally.")
...
def generate_single_file_report(ctx: dict, module_count: int):
    ...
    return {
        ...
        "semantic_analysis": {
            "mutability": [
                {"symbol": k, "mutated_args": v.get("mutated_args", []), "globals_mutated": v.get("globals_mutated", [])}
                for k, v in ctx.get("state_context", {}).items() if not v.get("is_pure", True)
            ],
            "side_effects": [
                {"symbol": k, "nonlocals": v.get("nonlocals_used", []), "self_mutations": v.get("self_mutations", []), "captured": v.get("captured_in_closures", [])}
                for k, v in ctx.get("state_context", {}).items() if not v.get("is_pure", True)
            ],
            "risks": [
                {"symbol": k, "complexity": v.get("metrics", {}).get("complexity", 1), "raises": v.get("metrics", {}).get("raises", [])}
                for k, v in ctx.get("function_context", {}).items() if v.get("metrics", {}).get("complexity", 1) > 5
            ]
        },
        "functions": ctx.get("function_context", {}),
