...
"""
repo_guardian/core/hotspots/normalization.py
...
def _log_normalize(values: dict[str, int]) -> dict[str, float]:
    ...
    return {
        key: round(math.log(1 + value) / denominator, 4)
...
def _geometric_map(incoming: dict[str, int], outgoing: dict[str, int]) -> dict[str, float]:
    ...
    values = {
        node: math.sqrt(incoming.get(node, 0) * outgoing.get(node, 0))
        for node in nodes
    }
