...
"""
repo_guardian/core/hotspots/engine.py
...
from .normalization import _percentile_map, _log_normalize, _geometric_map
from .classification import compute_hotspot_score, classify_module, _build_hotspot_explanation
...
def detect_hotspots(hard_edges: Dict[str, Set[str]]) -> List[dict]:
    ...
    log_map = _log_normalize({
        module: in_degree.get(module, 0) + out_degree.get(module, 0)
        ...
        if kind != "NORMAL":
            ...
            results.append({
                ...
                "explanation": _build_hotspot_explanation(
                    kind, impact, complexity,
                    log_map.get(module, 0.0), geometric_map.get(module, 0.0)
                ),
