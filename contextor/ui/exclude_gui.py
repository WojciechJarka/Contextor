...
from contextor.core.paths import ensure_dir, repo_key, state_dir
...
# User state lives outside the package directory, which is read-only in
# a normal installation. Files are keyed by the absolute repository path,
# so two projects that merely share a folder name stay independent.


def _excludes_dir() -> Path:
    return ensure_dir(state_dir() / "excludes")
...
def get_state_file():
    ...
    if not repo:
        return _excludes_dir() / "exclude_state.json"

    return exclude_state_file(repo)
...
def get_preset_file():
    ...
    if not repo:
        return _excludes_dir() / "exclude_presets.json"

    return _excludes_dir() / f"{repo_key(repo)}_presets.json"
...
def find_repo_root():
    ...
    from contextor.ui.path_memory import load_state
...
def run_exclude_window(parent=None):
    ...
    e_tooltip = HeaderTooltipManager(
        sub_label, "Files and directories temporarily excluded from analysis"
    )
    ...
    preset_label_var = tk.StringVar(
        value=f"Active Preset: {current_preset_name}"
        if current_preset_name
        else "Active Preset: None"
    )
    ttk.Label(
        win, textvariable=preset_label_var, style="Sub.TLabel", foreground=theme.PRIMARY
    ...
    def add_item():
        ...
        choice_win.title("Add exclusion")

        choice_win.geometry("260x150")
        choice_win.configure(bg=theme.BG)
        choice_win.transient(win)

        def add_directory():

            path = filedialog.askdirectory(initialdir=str(repo), parent=choice_win)
        ...
        def add_file():
            ...
            path = filedialog.askopenfilename(
                initialdir=str(repo), filetypes=[("All files", "*.*")], parent=choice_win
            )
        ...
        ttk.Button(
            choice_win, text="Add directory", style="Secondary.TButton", command=add_directory
        ).pack(padx=PAD_MD, pady=(PAD_MD, PAD_SM), fill="x")

        ttk.Button(choice_win, text="Add file", style="Secondary.TButton", command=add_file).pack(
            padx=PAD_MD, pady=(0, PAD_MD), fill="x"
        )
    ...
    def process_selected_path(path):
        ...
        try:
            ...
            messagebox.showwarning("Error", "Selected element is not in the repo", parent=win)
    ...
    def exclude_selected():
        ...
        if moved:
            ...
            messagebox.showinfo("Excluded", "Selected items were already excluded.", parent=win)
    ...
    def save_preset():
        ...
        def do_save():
            ...
            if name.lower() == "default":
                messagebox.showwarning(
                    "Error", "The name 'default' is reserved and read-only.", parent=dlg
                )
                return
            if name in presets:
        ...
        ttk.Button(btn_row, text="Cancel", command=dlg.destroy, style="Ghost.TButton").pack(
    ...
    def load_preset():
        ...
        ttk.Button(sel_win, text="Load", command=load_btn, style="Primary.TButton").pack(
            pady=PAD_SM
        )
    ...
    def delete_preset():
        ...
        if not presets:
            messagebox.showinfo(
                "Presets", "No saved presets found for this repository.", parent=win
            )
            return

        def on_select(name):
            confirm = messagebox.askyesno(
                "Confirm Delete", f"Delete preset '{name}'?", parent=sel_win
            )
            if confirm:
                del presets[name]
                save_presets_dict(presets)
        ...
        ttk.Button(sel_win, text="Delete", command=del_btn, style="Danger.Ghost.TButton").pack(
            pady=PAD_SM
        )
    ...
    b_rem = ttk.Button(
        list_frame, text="- Del", style="Danger.Ghost.TButton", command=remove_selected
    )
    ...
    b_exc = ttk.Button(
        state_frame, text="Exclude", style="Secondary.TButton", command=exclude_selected
    )
    b_exc.pack(side="left", padx=(0, PAD_SM))
    e_tooltip.bind_tooltip(b_exc, "Mark selected items as Excluded (soft-filter).")

    b_res = ttk.Button(
        state_frame, text="Restore", style="Secondary.TButton", command=restore_selected
    )
    b_res.pack(side="left", padx=(0, PAD_SM))
    e_tooltip.bind_tooltip(b_res, "Restore Excluded items to Active.")

    b_res_all = ttk.Button(
        state_frame, text="Restore all", style="Danger.Ghost.TButton", command=restore_all
    )
    ...
    b_auto = ttk.Button(
        state_frame,
        text="Auto-exclude non-Python",
        style="Secondary.TButton",
        command=auto_exclude_non_python,
    )
    ...
    b_def = ttk.Button(
        preset_actions, text="Default", command=load_default_preset, style="Secondary.TButton"
    )
    b_def.pack(side="left", padx=(0, PAD_SM))
    e_tooltip.bind_tooltip(b_def, "Restore to default (no active preset, clear exclusions).")

    b_sv = ttk.Button(
        preset_actions, text="Save Preset", command=save_preset, style="Secondary.TButton"
    )
    b_sv.pack(side="left", padx=(0, PAD_SM))
    e_tooltip.bind_tooltip(
        b_sv, "Save the current exclusion list as a reusable preset for this repo."
    )

    b_ld = ttk.Button(
        preset_actions, text="Load Preset", command=load_preset, style="Secondary.TButton"
    )
    b_ld.pack(side="left", padx=(0, PAD_SM))
    e_tooltip.bind_tooltip(b_ld, "Load a saved exclusion preset.")

    b_dl = ttk.Button(
        preset_actions, text="Delete Preset", command=delete_preset, style="Danger.Ghost.TButton"
    )
    b_dl.pack(side="left")
    ...
    ttk.Label(info_frame, text=info_text_1, foreground=theme.TEXT).pack(
    ...
    ttk.Label(info_frame, text=info_text_2, foreground=theme.TEXT).pack(
        anchor="w", padx=PAD_SM, pady=(0, PAD_SM)
    )

    def handle_close(is_confirm=False):
        changed_from_start = (
            set(candidates) != set(original_candidates) or excluded_set != original_excluded_set
        )
        ...
        if current_preset_name:
            ...
            preset_changed = (sorted(candidates) != sorted(p_data.get("candidates", []))) or (
                sorted(list(excluded_set)) != sorted(p_data.get("excluded", []))
            )
    ...
    ttk.Button(
        ...
        command=lambda: _on_close(is_confirm=True),
    ).pack(side="right")
