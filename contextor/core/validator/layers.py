"""
contextor/core/validator/layers.py
...
def validate_layer_rules(
    ...
    for module_id in modules:
        source_layer = get_layer(module_id)

        for dependency in hard_edges.get(
            module_id,
            set(),
        ):
            target_layer = get_layer(dependency)
            ...
            for (
                ...
                if source_layer == forbidden_source and target_layer == forbidden_target:
                    errors.append(
                        ValidationError(
                            kind="LAYER",
                            message=(f"{description}: {module_id} -> {dependency}"),
                            nodes=[
                                module_id,
                                dependency,
                            ],
    ...
    for module_id in modules:
        ...
        for dependency in hard_edges.get(
            ...
            for (
                ...
                if source_layer == forbidden_source and dependency.startswith(forbidden_prefix):
                    errors.append(
                        ValidationError(
