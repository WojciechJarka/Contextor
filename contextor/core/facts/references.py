"""
contextor/core/facts/references.py
...
class ReferenceGraph:
    ...
    def ensure(self, symbol_id: str):

        if symbol_id not in self.references:
            self.references[symbol_id] = SymbolReference()

    def get(self, symbol_id: str):

        return self.references.get(symbol_id, SymbolReference())
...
class ReferenceVisitor(ast.NodeVisitor):
    ...
    def visit_ImportFrom(self, node):
        ...
        for item in node.names:
            ...
            target = f"{node.module}.{item.name}" if node.module else item.name

            self.aliases[local] = target

            for symbol_id in _resolve_name(item.name, self.registry):
                self.graph.ensure(symbol_id)

                self.graph.references[symbol_id].imported_by.add(self.module_id)
    ...
    def visit_Import(self, node):
        ...
        for item in node.names:
            local = item.asname or item.name.split(".")[-1]
    ...
    def visit_ClassDef(self, node):
        ...
        for base in node.bases:
            name = _attribute_name(base)

            if not name:
                continue

            for symbol_id in _resolve_name(name.split(".")[-1], self.registry):
                self.graph.ensure(symbol_id)

                self.graph.references[symbol_id].inherited_by.add(self.module_id)
    ...
    def visit_Call(self, node):
        ...
        for symbol_id in _resolve_name(short, self.registry):
            self.graph.ensure(symbol_id)

            self.graph.references[symbol_id].called_by.add(self.module_id)
