"""
contextor/core/facts/symbols.py
...
class SymbolRegistry:
    ...
    def add(self, symbol: SymbolRecord):
        """
        Adds a symbol.
    ...
    def by_module(self, module_id: str) -> list[SymbolRecord]:

        return sorted(
            [s for s in self.symbols.values() if s.module_id == module_id], key=lambda x: x.id
        )

    def public_symbols(self) -> list[SymbolRecord]:

        return sorted([s for s in self.symbols.values() if s.public], key=lambda x: x.id)
