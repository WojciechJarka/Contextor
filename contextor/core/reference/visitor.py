...
class SymbolReferenceVisitor(ast.NodeVisitor):
            ...
            if len(node.args) >= 2:
                ...
                if match:
                    self.event_bound.add(
                        (match, getattr(node, "lineno", None), self._current_context())
                    )
        ...
        if called_name == "getattr" and len(node.args) >= 2:
            if isinstance(node.args[1], ast.Constant) and isinstance(node.args[1].value, str):
                ...
                _, dyn_match = _classify_match(
                    dynamic_name, resolved_dyn, self.target_symbols, self.aliases
                )
                if dyn_match:
                    self.called_ambiguous.add(
                        (dyn_match, getattr(node, "lineno", None), self._current_context())
                    )
        ...
        for arg in node.args:
            ...
            if arg_name:
                ...
                _, arg_match = _classify_match(
                    arg_name, resolved_arg, self.target_symbols, self.aliases
                )
                if arg_match:
                    self.called.add(
                        (arg_match, getattr(node, "lineno", None), self._current_context())
                    )
    ...
    def _resolve_instance_method(self, resolved):
        ...
        if candidate in self.target_symbols:
            ...
            return (
                candidate  # We'll return candidate instead of True to let caller add to self.called
            )
        return None
