...
def _split_layer_modules(
    ...
    for module_id, module_obj in modules.items():
        ...
        matches_package_name = (top_package == target_layer_name) or (
            mod_str.lower() == target_layer_name
        )
...
def generate_layer_report(
    ...
    hard_edges = getattr(
        graph, "hard_edges", graph.get("hard_edges", {}) if isinstance(graph, dict) else {}
    )
    soft_edges = getattr(
        graph, "soft_edges", graph.get("soft_edges", {}) if isinstance(graph, dict) else {}
    )
