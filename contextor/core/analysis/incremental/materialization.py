...
def ensure_topology_analytics(state: RepositoryAnalysisState) -> None:
    ...
    if not hasattr(state, "topology_metrics_state") or state.topology_metrics_state is None:
        state.topology_metrics_state = (
            "fresh" if bool(getattr(state, "topology_analytics", None)) else "deferred"
        )
    ...
    if (
        ...
        and not state.topology_analytics
        and getattr(state, "dependency_graph", None) is not None
    ):
        from contextor.core.reporting_engine.graph_analytics import compute_topology_analytics

        state.topology_analytics = compute_topology_analytics(
            state.dependency_graph.hard_edges,
            state.dependency_graph.soft_edges,
            state.metrics,
        )
