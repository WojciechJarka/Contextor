...
_CACHE: "OrderedDict[str, ProjectGraph]" = OrderedDict()
...
def _serialize_import(imp) -> tuple:
    """
    ...
    return (
        imp.module or "",
        getattr(imp, "level", 0),
        tuple(sorted(getattr(imp, "names", []))),
        getattr(imp, "is_from_import", False),
        getattr(imp, "is_local", False),
        getattr(imp, "type_only", False),
    )
...
def _hash_modules(modules: dict) -> str:
    """
    ...
    for module_id in sorted(modules.keys()):
        ...
        imports = [_serialize_import(imp) for imp in module.imports]
...
def _cache_put(key: str, graph: ProjectGraph):
    """
...
def get_cached_graph(modules: dict, builder_fn) -> tuple[ProjectGraph, bool]:
    """
    ...
    cache_key = _hash_modules(modules)
    ...
    graph: ProjectGraph = builder_fn(modules)
