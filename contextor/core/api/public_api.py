...
def _collect_category(symbols, category):
    """
    ...
    for item in symbols.get(category, []):
        if _is_public_name(_symbol_name(item)):
            result.append(item)
...
def extract_public_api(symbols: dict) -> list[str]:
    """
    ...
    for category in (
        ...
        public.extend(_collect_category(symbols, category))
...
build_public_api = extract_public_api
