...
class PersistentIdentityRegistry:
    ...
    def _lock(self):
        ...
        if sys.platform == "win32":
            import msvcrt
            msvcrt.locking(self._lock_file_obj.fileno(), msvcrt.LK_LOCK, 1)
        else:
            import fcntl
            fcntl.flock(self._lock_file_obj, fcntl.LOCK_EX)

    def _unlock(self):
        if self._lock_file_obj:
            if sys.platform == "win32":
                import msvcrt
                msvcrt.locking(self._lock_file_obj.fileno(), msvcrt.LK_UNLCK, 1)
            else:
                import fcntl
                fcntl.flock(self._lock_file_obj, fcntl.LOCK_UN)
    ...
    def _load_json(self, name: str) -> Dict[str, Any]:
        ...
        # Default structures
        if "registry" in name:
            return {"schema_version": self.SCHEMA_VERSION, "path_to_id": {}, "id_to_path": {}}
        elif "references" in name:
            return {"schema_version": self.SCHEMA_VERSION, "id_to_reports": {}, "report_to_ids": {}}
        else:
            return {"schema_version": self.SCHEMA_VERSION}

    def _load_all(self):
        self._state = {name: self._load_json(name) for name in self.files}
        
    def _recover_transaction(self):
        if self.transaction_file.exists():
            try:
                tx = json.loads(self.transaction_file.read_text(encoding="utf-8"))
    ...
    def transaction(self):
        ...
        try:
            ...
            for name, path in self.files.items():
                tmp_path = path.with_suffix(".json.tmp")
                data_str = json.dumps(self._state[name], indent=2)
            ...
            tx_data = {
                ...
                "files": list(self.files.keys()), # Ordered intentionally by python dict 3.7+
                "status": "committing"
            }
            self.transaction_file.write_text(json.dumps(tx_data), encoding="utf-8")
            with open(self.transaction_file, "a") as f:
                f.flush()
                os.fsync(f.fileno())
                
            # Atomic renames in exact order (slots, recovery, references, then registries)
    ...
    def list_modules(self) -> list[str]:
        return list(self._state["module_registry"]["path_to_id"].keys())
    ...
    def sync_with_workspace(self, current_modules: Set[str], current_artifacts: Set[str]):
    ...
    def _sync_kind(self, kind: str, current_items: Set[str]):
        registry = self._state[f"{kind}_registry"]
        recovery = self._state[f"{kind}_recovery"]
        
        active_paths = list(registry["path_to_id"].keys())
        ...
        for path in current_items:
            if path not in registry["path_to_id"]:
                ...
                for rec_id, rec_data in list(recovery.items()):
                    if rec_id == "schema_version": continue
                    if rec_data.get("path" if kind == "module" else "name") == path:
                        if rec_id not in registry["id_to_path"]: # Ensure the ID isn't somehow taken
                            restored_id = rec_id
                        break
    ...
    def register_report_references(self, report_name: str, used_ids: List[str]):
    ...
    def run_garbage_collector(self, max_module_recovery_bytes: int = 250 * 1024, max_artifact_recovery_bytes: int = 100 * 1024):
        self._gc_kind("module", max_module_recovery_bytes)
        self._gc_kind("artifact", max_artifact_recovery_bytes)

    def _gc_kind(self, kind: str, max_bytes: int):
        recovery = self._state[f"{kind}_recovery"]
        
        # Calculate current size (approximate using json.dumps)
        current_size = len(json.dumps(recovery).encode("utf-8"))
        ...
        def get_date(item):
            return item[1].get("removed_at", "1970-01-01")
            
        sorted_orphans = sorted(
            [(k, v) for k, v in recovery.items() if k != "schema_version"],
            key=get_date
        )
