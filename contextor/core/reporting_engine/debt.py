...
def _compute_cluster_penalty(
    clusters: list[list[str]], hotspots: list[dict], thresholds: dict
) -> int:
    config_modules = {
        hotspot.get("module") for hotspot in (hotspots or []) if hotspot.get("type") == "CONFIG_HUB"
    }
...
def compute_debt(
    ...
    interpretation = {
        "label": label,
        "breakdown": {
            "collisions": f"{collision_count} real collisions (+{collision_penalty})"
            if collision_count
            else "none",
            "soft_edges": f"{soft_edges_count} soft edges (+{soft_edge_penalty})"
            if soft_edges_count
            else "none",
            "cycles": f"{cycle_penalty} nodes in cycles (+{cycle_penalty * 2})"
            if cycle_penalty
            else "none",
            "hotspots": f"{hotspot_penalty} above critical threshold (+{hotspot_penalty})"
            if hotspot_penalty
            else "none above critical threshold",
        },
    }
