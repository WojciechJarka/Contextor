"""
contextor/core/reporting_engine/risk_signals.py
...
def _compute_module_risk(metrics: dict, graph: dict) -> dict:
    ...
    for node in sorted(nodes):
        ...
        in_deg = sum(node in targets for targets in graph["hard_edges"].values())
        out_deg = len(deps)
...
def _compute_risk_summary(risk_map: dict, critical_score: float) -> dict:
    ...
    critical = sorted(module for module, score in risk_map.items() if score >= critical_score)
