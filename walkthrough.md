...
## EXACT WATCHER ERROR-RECOVERY REGRESSIONS
VERDICT=IMPLEMENTATION_PASS
PRESEND_UNBOUNDLOCALERROR=0
PRESEND_UPDATE_COUNT_AT_FAILURE=0
PRESEND_FALSE_AMBIGUOUS=0
WATCHER_THREAD_SURVIVES_PRESEND_FAILURE=PASS
NEXT_AUTOMATIC_POLL_AFTER_PRESEND_FAILURE=PASS
FIRST_PENDING_CHANGE_RECONCILED_EXACTLY_ONCE=PASS
SECOND_CHANGE_RECONCILED_EXACTLY_ONCE=PASS
UPDATE_ATTEMPTED_SCOPE=PER_CANDIDATE
PATH_A_UPDATE_COUNT=1
PATH_B_UPDATE_COUNT_BEFORE_RECOVERY=0
...
COMPLETE_RAW_DIFFS=YES
diff --git a/tests/test_live_watcher_startup_reconciliation.py b/tests/test_live_watcher_startup_reconciliation.py
index 62e083a..8802fe8 100644
--- a/tests/test_live_watcher_startup_reconciliation.py
+++ b/tests/test_live_watcher_startup_reconciliation.py
@@ -920,21 +920,39 @@ def test_background_watcher_recovers_from_ambiguous_update_without_restart(tmp_p
...
+    identity = PersistentIdentityRegistry(str(repo))
+    manager = FileStateManager(str(repo_cache_dir(repo))); manager.update_state(str(source)); manager.save(identity.repo_id, revision=0)
+    state = SimpleNamespace(revision=0, state_id=identity.repo_id, modules={"module": object()})
+    updates = []
+    def updater(current, path):
+        updates.append(path); manager.update_state(path); manager.save(identity.repo_id, revision=current.revision + 1)
+        return SimpleNamespace(status="UPDATED", file_path=path)
+    server = CanonicalLiveServer(state, revision=0, updater=updater)
+    thread = threading.Thread(target=server.serve_forever, daemon=True); thread.start()
+    client = LiveStateClient(server.endpoint)
+    watcher = DesktopLiveWatcher(repo, client, interval=0.02)
+    original_snapshot = client.snapshot; failed = threading.Event(); restored = threading.Event(); armed = threading.Event()
+    def snapshot_once():
+        if armed.is_set() and not failed.is_set(): failed.set(); raise ConnectionError("presend")
+        return original_snapshot()
+    client.snapshot = snapshot_once
+    armed.set()
+    source.write_text("VALUE=2\n", encoding="utf-8")
     watcher.start()
     try:
...
+        assert updates == [] and str(source) not in watcher._ambiguous_updates
         assert watcher._thread is not None and watcher._thread.is_alive()
         assert watcher._stop.is_set() is False
+        restored.set()
+        deadline = time.monotonic() + 3
+        while len(updates) < 1 and time.monotonic() < deadline: time.sleep(0.02)
...
## LITERAL WATCHER REGRESSIONS
VERDICT=IMPLEMENTATION_PASS
PRESEND_UPDATE_COUNT_AT_FAILURE=0
PRESEND_FALSE_AMBIGUOUS=0
FIRST_PENDING_CHANGE_RECONCILED_EXACTLY_ONCE=YES
SECOND_CHANGE_RECONCILED_EXACTLY_ONCE=YES
PATH_A_UPDATE_COUNT_AFTER_FAILURE=1
PATH_B_UPDATE_COUNT_AFTER_FAILURE=0
PATH_B_FALSE_AMBIGUOUS=YES
PATH_A_UPDATE_COUNT_FINAL=1
PATH_B_UPDATE_COUNT_FINAL=1
FINAL_QUIESCENT_POLL=PASS
FULL_IPC_GATE=47_PASSED
H3A_RESULT=27_PASSED
FILES_CHANGED=tests/test_live_watcher_startup_reconciliation.py
COMPLETE_RAW_DIFFS=YES
diff --git a/tests/test_live_watcher_startup_reconciliation.py b/tests/test_live_watcher_startup_reconciliation.py
index 62e083a..3e7ce9f 100644
--- a/tests/test_live_watcher_startup_reconciliation.py
+++ b/tests/test_live_watcher_startup_reconciliation.py
@@ -920,32 +920,66 @@ def test_background_watcher_recovers_from_ambiguous_update_without_restart(tmp_p
...
+    update_calls = []; first_done = threading.Event(); second_done = threading.Event(); presend = threading.Event(); allow = threading.Event()
+    original_update = client.update_file
...
+        allow.wait(2); update_calls.append(path)
...
-        assert watcher._thread is not None and watcher._thread.is_alive()
-        assert watcher._stop.is_set() is False
+        assert presend.wait(3); assert update_calls == []; assert str(source) not in watcher._ambiguous_updates
+        assert watcher._thread is not None and watcher._thread.is_alive(); assert not watcher._stop.is_set()
+        allow.set(); assert first_done.wait(3); assert update_calls == [str(source)]
+        source.write_text("VALUE=3\n", encoding="utf-8"); assert second_done.wait(3); assert update_calls == [str(source), str(source)]
+        time.sleep(0.1); assert len(update_calls) == 2; assert watcher._thread.is_alive()
     finally:
-        watcher.stop()
...
+    assert str(b) not in watcher._ambiguous_updates; assert str(a) in watcher._snapshot
+    watcher._recover_client = lambda: client
+    assert watcher.poll_once() == [str(b)]
+    assert counts[str(a)] == 1 and counts[str(b)] == 1; assert str(b) not in watcher._ambiguous_updates
+    assert watcher.poll_once() == []
## FINAL TWO TEST CORRECTIONS
VERDICT=FIX_REQUIRED
EXACT_TEST_1=test_presend_connection_failure_does_not_raise_unboundlocal_or_kill_worker: PASS
EXACT_TEST_2=test_update_attempted_flag_is_path_local_for_multiple_candidates: PASS
...
PATH_A_BASELINE_ADVANCED_AFTER_FIRST_POLL=PARTIAL (size evidence; mtime is platform-volatile)
PATH_B_BASELINE_RETAINED_AFTER_FAILURE=PASS
PATH_B_FALSE_AMBIGUOUS=0
PATH_A_REPLAY_AFTER_B_FAILURE=0
PATH_B_FINAL_UPDATE_COUNT=1
FINAL_QUIESCENT_POLL=PASS
